...
BACK_IGNORE = 5  # 忽略视频尾部的时长阈值
FRONT_IGNORE = 2  # 忽略视频头部的时长阈值
...
def video_test(video_path):
    ...
    if cap.isOpened():
        ...
        serial_freeze = 0
        pre = None
        cap.set(cv2.CAP_PROP_POS_FRAMES, fps*FRONT_IGNORE)  # 从视频第二秒开始检测
        # counter = 0
        while num:
            is_read, frame = cap.read()
            # is_read, frame = cap.read()
            if num % 2 == 0 and is_read:
                ...
                if is_black:
                    ...
                    if serial_black >= BLACK_TIME * fps:
                        ...
                        with open('logs/black_log2.txt','a') as f:
                ...
                if is_freeze:
                    ...
                    if serial_freeze >= FREEZE_TIME * fps:
                        ...
                        with open('logs/freeze_log2.txt','a') as f:
